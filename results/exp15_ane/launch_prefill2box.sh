...
# ── 5) 준비 대기 (하드 시한 — 침묵 금지) ────────────────────────────
# /health 는 모델 로드 *전에* 응답한다(httpd 가 먼저 뜬다). 준비 판정은
# 프리필-2박스 연결 로그이며, ANE 모드에서는 부착 로그까지 있어야 한다 —
# /health 만 보고 판정하면 로드 중인 서버를 준비된 것으로 읽는다.
t0=$(date +%s)
while true; do
    if grep -q 'prefill_2box\] connected' $LOG 2>/dev/null; then
        if [[ $ANE == 1 ]] && ! grep -q 'serve-ane' $LOG; then
            fail "프리필-2박스는 연결됐으나 ANE 부착 로그가 없다 — 벤더 진입 순서 이탈"
        fi
        curl -s -m 5 "http://127.0.0.1:$PORT/health" >/dev/null 2>&1 || \
            fail "연결 로그는 있으나 /health 무응답"
        say "✓ 준비 완료 ($(( $(date +%s) - t0 ))s)"
        [[ $ANE == 1 ]] && say "  $(grep -m1 'serve-ane' $LOG)"
        say "  $(grep -m1 'prefill_2box\] connected' $LOG | sed 's/.*INFO - //')"
        break
    fi
    ...
    (( $(date +%s) - t0 > HEALTH_DEADLINE )) && { tail -25 $LOG; \
        ...
        fail "준비 대기 ${HEALTH_DEADLINE}s 초과"; }
    sleep 3
done
