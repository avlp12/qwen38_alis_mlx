...
class V4Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs, layer_idx: int):
        ...
        # 레퍼런스 정합(ref Attention.__init__): 층의 메인 rope 는 압축 층이면
        # compress_rope_theta + YaRN, ratio-0 층이면 rope_theta + YaRN 없음.
        # (기존: 전 층 rope_theta+YaRN → 위상 오차가 위치에 비례해 성장,
        #  약 2K 위치부터 생성 붕괴 — llama.cpp 대조로 확정한 실측.)
    ...
    def __call__(self, x: mx.array, mask=None, cache=None):
        ...
        if self.compress_ratio:
            ...
            if pool is not None:
                ckv = pool
                # wsdpa 프리필은 인덱서 게더를 우회하고 전체 풀을 per-query
                # 가시성(pvis)으로 방문한다(청크-수준 topk 의 인과 위반 제거).
                # 스톡 폴백 경로에서만 인덱서를 계산한다.
                _wsdpa_pool = ckv
        ...
        # ── wsdpa (oMLX 이식): 프리필 청크의 창+풀 어텐션을 가시 행만 방문하는
        # 융합 커널로 계산. 인덱서-게더 층(_wsdpa_pool None + compressed 존재)과
        # 디코드(S==1)는 스톡 경로 유지. 커널 실패 시 자동 영구 폴백.
        if (
            S > 1
            and B == 1
            and cache is not None
            and (compressed_k is None or _wsdpa_pool is not None)
        ):
            from .dsv4_wsdpa import wsdpa_prefill

            _o = wsdpa_prefill(
                q,
                k,
                _wsdpa_pool,
                self.attn_sink.astype(mx.bfloat16),
                self.scale,
                offset,
